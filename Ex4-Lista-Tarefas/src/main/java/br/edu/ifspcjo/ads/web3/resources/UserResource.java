...
import br.edu.ifspcjo.ads.web3.service.UserService;
...
public class UserResource 
{
	...
	// o Spring injeta automaticamente uma instância do UserRepository
	@Autowired 
	...
	*/
	...
	// Atualiza todos os dados de um user existente
	@PutMapping("/{id}")
	@ResponseStatus(HttpStatus.NO_CONTENT) // retornará 204 No Content (ou seja, sem corpo, só sinalizando que deu certo)
	public void update(@PathVariable Long id, @Valid @RequestBody User user) {
	    userService.update(id, user); // chama o método de atualizar
	}
	
	// testar Postman
	// PUT - com dados enviados no body
	
	
	// Atualiza o status ativo de um user existente
	@PutMapping("/{id}/active")
	@ResponseStatus(HttpStatus.NO_CONTENT) // retornará 204 No Content (ou seja, sem corpo, só sinalizando que deu certo)
	public void updateActiveProperty(@PathVariable Long id, @RequestBody Boolean active) {
		userService.updateActiveProperty(id, active); // chama o método do UserService para realmente atualizar o user
	}
